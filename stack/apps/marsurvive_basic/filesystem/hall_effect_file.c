...
error_t hall_effect_files_initialize()
{
    ...
    hw_gpio_configure_pin_stm(HALL_EFFECT_PIN, &input_config);
    hw_gpio_configure_interrupt(
        HALL_EFFECT_PIN, GPIO_FALLING_EDGE | GPIO_RISING_EDGE, &hall_effect_interrupt_callback, NULL);
    hw_gpio_enable_interrupt(HALL_EFFECT_PIN);

}

static void hall_effect_interrupt_callback(void* arg) { 
    sched_post_task(&hall_effect_sched_task); }
static void hall_effect_sched_task()
{
    hall_effect_file_t hall_effect_file = { .mask = hw_gpio_get_out(HALL_EFFECT_PIN) };
    if (prev_state != hall_effect_file.mask)
    {
        d7ap_fs_write_file(HALL_EFFECT_FILE_ID, 0, hall_effect_file.bytes, HALL_EFFECT_FILE_SIZE, ROOT_AUTH);
        prev_state = hall_effect_file.mask;
    }
}
