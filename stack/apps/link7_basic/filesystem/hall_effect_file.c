...
error_t hall_effect_files_initialize()
{
    ...
    hw_gpio_configure_pin_stm(INTERRUPT_TEST_PIN, &input_config);
    hw_gpio_configure_interrupt(
        INTERRUPT_TEST_PIN, GPIO_FALLING_EDGE | GPIO_RISING_EDGE, &hall_effect_interrupt_callback, NULL);
    hw_gpio_enable_interrupt(INTERRUPT_TEST_PIN);
}
...
static void hall_effect_sched_task()
{
    hall_effect_file_t hall_effect_file = { .mask = hw_gpio_get_out(INTERRUPT_TEST_PIN) };
